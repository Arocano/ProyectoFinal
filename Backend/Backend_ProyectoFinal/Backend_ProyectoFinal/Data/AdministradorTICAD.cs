...
namespace Backend_ProyectoFinal.Data
{
    public class AdministradorTICAD
    {
        public static List<AdministradorTI> ObtenerEmpleadosTI()
        {
            ...
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    admin.Add(new AdministradorTI(usuario, pass, nombre, apellido, correo));
                }
                return admin;
            }
            catch (Exception ex)
        ...
        public static AdministradorTI ObtenerEmpleadoTI(string user)
        {
            try
            {
                ...
                while (dr.Read())
                {
                    ...
                    string apellido = dr["APE_USU"].ToString();
                    string correo = dr["COR_USU"].ToString();
                    e = new AdministradorTI(usuario, pass, nombre, apellido, correo);
                }
                return e;
            }
            catch (Exception ex)
        ...
        public static bool ModificarContraseniaAdministradorTI(string user, AdministradorTI a)
        {
            try
            {
                Conexion con = new Conexion();
                string sql = "UPDATE USUARIOS SET PAS = '" + a.Contrasenia +"' " +
                    "WHERE USU = '" + user + "';";
                SqlCommand comand = new SqlCommand(sql, con.Conectar());
                int filasAfectadas = comand.ExecuteNonQuery();
                if (filasAfectadas == 1)
